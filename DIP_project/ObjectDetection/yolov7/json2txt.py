...
def json2txt_3_class(json_dir):
    """
    將json檔案轉換成txt檔案, class: Cancer=0, Mix=1, Warthin=2
    """
    json_files = glob.glob(os.path.join(json_dir, '*.json'))
    print(f"{json_dir}: {len(json_files)} annotations")
    for file in json_files:
        ...
        class_name = None
        for i in range(len(data['shapes'])):
            if data['shapes'][i]['label'] == 'cancer':
                class_name = '0'
            elif data['shapes'][i]['label'] == 'mix':
                class_name = '1'
            elif data['shapes'][i]['label'] ==  'warthin':
                class_name = '2'
            else :  # only need to detect class: Cancer=0, Mix=1, Warthin=2
                continue
